...
#   - The encoder receives [vars..., calibration] without reordering columns.
#   - The decoder receives ONLY the target (no exogenous variables).
...
CONFIG = {
    ...
    "results_dir":  "/results",
    "results_file": "results_seq2seq_arch2.txt",
...
def split_sequences(
    ...
    """
    Generate sliding windows for Seq2Seq prediction (decoder without future exogenous variables).
    ...
    Expected column layout in `sequences`: [var1, ..., calibration, target]
     ...
     ── ENCODER / DECODER INPUT COLUMN ORDER ──────────────────────────────────
    The encoder receives [var1, var2, ..., calibration] (original column order).
    The decoder receives ONLY the target column [target].
    ...
    Returns:
        ...
        dec_X  (N, n_outputs, 1)              ← [target]               decoder input
...
class Seq2Seq(Model):
    ...
    def call(self, encoder_inputs, decoder_inputs,
        ...
        Args:
            ...
            decoder_inputs : (batch, n_outputs, target)
...
def train_seq2seq(
    ...
    If save_weights=True, the following files are written:
        <results_dir>/<xtal_id>_seq2seq_arch2_<mode>_<n_steps>_<n_outputs>_<norm>/model_<n_steps>steps.weights.h5
        <results_dir>/<xtal_id>_seq2seq_arch2_<mode>_<n_steps>_<n_outputs>_<norm>/scaler_X_<n_steps>steps.pkl
        <results_dir>/<xtal_id>_seq2seq_arch2_<mode>_<n_steps>_<n_outputs>_<norm>/scaler_y_<n_steps>steps.pkl
        <results_dir>/<xtal_id>_seq2seq_arch2_<mode>_<n_steps>_<n_outputs>_<norm>/n_vars.txt
    ...
    if save_weights:
        ...
        weights_dir = os.path.join(
            results_dir,
            f"{xtal_str}_seq2seq_arch2_{mode}_{n_steps}_{n_outputs}_{norm_str}"
        )
...
def load_model(cfg=CONFIG):
    ...
    weights_dir = os.path.join(
        cfg["results_dir"],
        f"{xtal_str}_seq2seq_arch2_{mode_str}_{n_steps}_{n_outputs}_{norm_str}"
    )
    ...
    n_features_enc = len(d["variables"]) + 1   # [var1, var2, ..., calibration]
    n_features_dec = 1                         # Only target
...
if __name__ == "__main__":
    ...
    # Expected folder example: <results_dir>/30600_seq2seq_arch2_forced_48_48_minmax/
'''
