# =============================================================================
# lstm_multioutput_bidirectional.py
# Direct multi-output LSTM bidirectional model for time-series forecasting.
...
CONFIG = {
    ...
    "results_dir":  "results",
    "results_file": "lstm_multi_bi.txt",
...
from tensorflow.keras.layers import Dense, Dropout, BatchNormalization, LSTM, Input, Bidirectional
...
def build_model(
    ...
    """
    Build a multi-output Sequential LSTM bidirectional model.
...
def train_model(
    ...
    If save_weights=True, the following files are written:
        <results_dir>/<xtal_id>_multi_bi_<n_steps>_<n_outputs>_<norm>/model_<n_steps>steps.weights.h5
        <results_dir>/<xtal_id>_multi_bi_<n_steps>_<n_outputs>_<norm>/scaler_X_<n_steps>steps.pkl
        <results_dir>/<xtal_id>_multi_bi_<n_steps>_<n_outputs>_<norm>/scaler_y_<n_steps>steps.pkl
        <results_dir>/<xtal_id>_multi_bi_<n_steps>_<n_outputs>_<norm>/n_vars.txt
    ...
    if save_weights:
        ...
        weights_dir = os.path.join(
            results_dir, f"{xtal_str}_multi_bi_{n_steps}_{n_outputs}_{norm_str}"
        )
...
def load_model(cfg=CONFIG):
    ...
    weights_dir = os.path.join(
        cfg["results_dir"], f"{xtal_str}_multi_bi_{n_steps}_{n_outputs}_{norm_str}"
    )
...
if __name__ == "__main__":
    ...
    # Expected folder example: <results_dir>/30600_multi_bi_96_96_minmax/
'''
