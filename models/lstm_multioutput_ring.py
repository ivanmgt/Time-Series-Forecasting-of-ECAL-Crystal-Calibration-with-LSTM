# =============================================================================
# lstm_multioutput_ring.py
# Multi-output LSTM model WITHOUT Embedding for multi-crystal forecasting
...
CONFIG = {
    ...
    "results_dir":  "/results",
    "results_file": "results_multi_ring.txt",
...
def build_model(
    ...
    """
    Build a multi-output LSTM model without an Embedding layer.
...
def train_model(
    ...
    """
    Train the multi-output LSTM over all crystals in df_train, with independent
    per-crystal normalization. Uses model.fit() with Keras EarlyStopping.

    If save_weights=True, the following files are written:
        <results_dir>/<ring>_multi_<n_steps>_<n_outputs>_<norm>/model_<n_steps>steps.weights.h5
        <results_dir>/<ring>_multi_<n_steps>_<n_outputs>_<norm>/scaler_X_<xtal_id>.pkl
        <results_dir>/<ring>_multi_<n_steps>_<n_outputs>_<norm>/scaler_y_<xtal_id>.pkl
        <results_dir>/<ring>_multi_<n_steps>_<n_outputs>_<norm>/n_vars.txt
    ...
    if save_weights:
        ...
        weights_dir = os.path.join(
            results_dir, f"{ring_str}_multi_{n_steps}_{n_outputs}_{norm_str}"
        )
...
def load_model(cfg=CONFIG):
    ...
    weights_dir = os.path.join(
        cfg["results_dir"], f"{ring_str}_multi_{n_steps}_{n_outputs}_{norm_str}"
    )
...
if __name__ == "__main__":
    ...
    # Expected folder example: <results_dir>/ring_1_multi_24_24_minmax/
'''
