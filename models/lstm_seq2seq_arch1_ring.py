# =============================================================================
# lstm_seq2seq_arch1_ring.py
# LSTM Seq2Seq neural network (decoder with exogenous variables) for
...
CONFIG = {
    ...
    "results_dir":  "/results",
    "results_file": "results_seq2seq_arch1_ring_v2.txt",
...
# ──────────────────────────────────────────────────────────────────────────────
# 5. WINDOWING FOR SEQ2SEQ ARCHITECTURE 1 (decoder with exogenous variables)
# ──────────────────────────────────────────────────────────────────────────────
...
def split_sequences(
    ...
    """
    Generate sliding windows for Seq2Seq arch1 (decoder with exogenous variables).
    ...
    Returns:
        ...
        dec_X  (N, n_outputs, 1+n_dec_vars)   ← [target, vars...]      for decoder
    ...
    for i in range(0, T - n_steps - n_outputs + 1, stride):
        ...
        # ── Encoder: reorder to [calibration, other_vars] ──
...
# ──────────────────────────────────────────────────────────────────────────────
# 6. SEQ2SEQ ARCHITECTURE 1 MODEL
# ──────────────────────────────────────────────────────────────────────────────

class Seq2Seq(Model):
    """
    Stacked Encoder–Decoder LSTM with teacher forcing / scheduled sampling.
    The decoder receives [target, var1, var2, ...] at each step (exogenous variables).
    ...
    def call(self, encoder_inputs, decoder_inputs,
        ...
        decoder_inputs : (batch, n_outputs, n_dec_features)  ← [target, vars...]
        """
        ...
        dec_feat   = tf.shape(dec_inputs)[2]
        ...
        for t in range(self.n_outputs):
            ...
            pred = self.out_dense(out)
            outputs.append(pred)

            # Next input: pred + future exogenous variables (if any)
            # safe_t clamps the index so BOTH branches of tf.cond are valid
            # during @tf.function tracing, avoiding out-of-bounds access.
            def pred_next():
                ...
                exog_next = tf.cond(
                    ...
                    lambda: tf.zeros([batch_size, dec_feat - 1], dtype=dec_inputs.dtype),
                )
                return tf.concat([pred, exog_next], axis=-1)

            if training:
                if per_sample:
                    # safe_t clamps the index so @tf.function can trace
                    # both branches without out-of-bounds when t = n_outputs - 1.
                    mask     = tf.cast(
                    ...
                    safe_t   = tf.minimum(t + 1, dec_len - 1)
                    teacher  = dec_inputs[:, safe_t, :]
                    has_next = tf.cast(tf.less(t + 1, dec_len), dec_inputs.dtype)
                    teacher  = teacher * has_next   # zero out at the last step
                    mask_full = tf.tile(mask, [1, tf.shape(teacher)[1]])
                    dec_input = mask_full * teacher + (1.0 - mask_full) * pred_next()
                else:
                    # Global per-sequence decision — fully tensorized for
                    # @tf.function compatibility when per_sample=False.
                    # A plain Python `if use_teacher` fails during graph tracing
                    # because TF cannot evaluate a Tensor as a Python bool;
                    # tf.cond resolves both branches in the graph without Python eval.
...
def train_seq2seq(
    ...
    If save_weights=True, the following files are written:
        <results_dir>/<ring>_seq2seq_arch1_<mode>_<n_steps>_<n_outputs>_<norm>/model_<n_steps>steps.weights.h5
        <results_dir>/<ring>_seq2seq_arch1_<mode>_<n_steps>_<n_outputs>_<norm>/scaler_X_<xtal_id>.pkl
        <results_dir>/<ring>_seq2seq_arch1_<mode>_<n_steps>_<n_outputs>_<norm>/scaler_y_<xtal_id>.pkl
        <results_dir>/<ring>_seq2seq_arch1_<mode>_<n_steps>_<n_outputs>_<norm>/n_vars.txt
    ...
    if save_weights:
        ...
        weights_dir = os.path.join(
            results_dir, f"{ring_str}_seq2seq_arch1_{mode}_{n_steps}_{n_outputs}_{norm_str}"
        )
...
def load_model(cfg=CONFIG):
    ...
    weights_dir = os.path.join(
        cfg["results_dir"],
        f"{ring_str}_seq2seq_arch1_{mode_str}_{n_steps}_{n_outputs}_{norm_str}"
    )
    ...
    n_dec_features = len(d["variables"]) + 1   # exogenous vars + target
...
if __name__ == "__main__":
    ...
    run_single(df_train, df_test, CONFIG)   
       # ← default mode
    ...
    # Expected folder: <results_dir>/ring_1_seq2seq_arch1_forced_48_48_minmax/
'''
